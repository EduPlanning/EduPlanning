...
<div class="page-header">
    ...
    <div style="display:flex;gap:.5rem;flex-wrap:wrap">
        ...
        <button class="btn btn-primary" id="btnAddCreneau" style="display:none"><i class='bx bx-plus'></i> Ajouter</button>
    </div>
</div>
...
let allCreneaux   = [];
let isAdmin       = false;
let allMatieres   = [], allEnseignants = [], allSalles = [], allGroupes = [];
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (isAdmin) document.getElementById('btnAddCreneau').style.display = 'flex';
...
document.getElementById('prevWeek').addEventListener('click', () => {
    currentMonday = addDays(currentMonday, -7); renderWeek();
});
document.getElementById('nextWeek').addEventListener('click', () => {
    currentMonday = addDays(currentMonday, 7); renderWeek();
});
document.getElementById('todayBtn').addEventListener('click', () => {
    currentMonday = getMonday(new Date()); renderWeek();
});
...
async function loadResources() {
    const [m, e, s, g] = await Promise.all([
        ...
        fetch(`${BASE}/users/get_all.php?role=enseignant`).then(r=>r.json()),
...
function showDetail(id) {
    ...
    body.innerHTML = `
        ...
        ${isAdmin ? `<div class="form-actions" style="margin-top:1rem">
...
async function deleteCreneau(id) {
    ...
    showAlert(document.getElementById('alertContainer'),
        data.message === 'Créneau supprimé' ? 'success' : 'danger',
        data.message);
    renderWeek();
}
...
document.getElementById('btnSaveCreneau').addEventListener('click', async () => {
    ...
    const payload = { date_cours, heure_debut, heure_fin, matiere_id, enseignant_id,
                      salle_id, groupe_id, type, recurrent,
    ...
    if (data.conflits) {
        const names = { enseignant: 'enseignant', salle: 'salle', groupe: 'groupe' };
